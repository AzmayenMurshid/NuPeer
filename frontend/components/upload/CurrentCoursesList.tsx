...
export function CurrentCoursesList({
  courses,
  onEdit,
  onDelete,
  isDeleting,
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Add courses you're currently taking
          </p>
        </div>
      </div>
      {courses.length === 0 ? (
        ...
        <>
            ...
            {courses.map((course) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 ml-4">
                  ...
                  <button
                    onClick={() => {
                      if (confirm(`Are you sure you want to remove ${course.course_code}?`)) {
                        onDelete(course.id)
                      }
                    }}
                    disabled={isDeleting || isUpdating}
          ...
          {deleteError && (
            <div className="mt-4 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded text-red-700 dark:text-red-400 text-sm">
              <p className="font-medium mb-1">Failed to remove course</p>
              <p className="text-xs">
                {(deleteError as any)?.response?.data?.detail || 'Please try again.'}
              </p>
            </div>
          )}
        </>
