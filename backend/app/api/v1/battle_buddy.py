...
async def get_all_teams(
    ...
    teams = (
        db.query(BattleBuddyTeam)
        .options(joinedload(BattleBuddyTeam.members).joinedload(BattleBuddyMember.user))
        .order_by(BattleBuddyTeam.points.desc(), BattleBuddyTeam.team_name)
        .all()
    )
...
async def list_teams(
    ...
    # Optimized: Use subquery for member count to avoid N+1 queries (O(n) instead of O(n*m))
    member_counts = (
        db.query(
            BattleBuddyMember.team_id,
            func.count(BattleBuddyMember.id).label('member_count')
        )
        .group_by(BattleBuddyMember.team_id)
        .subquery()
    )
    ...
    teams = (
        ...
        .outerjoin(member_counts, BattleBuddyTeam.id == member_counts.c.team_id)
        .order_by(BattleBuddyTeam.points.desc(), BattleBuddyTeam.team_name)
        .all()
    )
...
async def get_my_team(
    ...
    # Optimized: Single query with joinedload to get team and all members with users (O(1) instead of O(n))
    team = (
        db.query(BattleBuddyTeam)
        .join(BattleBuddyMember, BattleBuddyTeam.id == BattleBuddyMember.team_id)
        .filter(BattleBuddyMember.user_id == current_user.id)
        .options(joinedload(BattleBuddyTeam.members).joinedload(BattleBuddyMember.user))
        .first()
    )
